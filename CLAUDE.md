...
End-to-end sync check — 63 checks over the real API, Redis and sockets (needs all of the
above running):
...
- **Removing someone marks the row, it does not delete it.** `DELETE /rooms/:code/members/:memberId` (host only) stamps `RoomMember.removedAt`, because `Message.memberId` references that row and chat keeps its attribution. It also pushes a `member`-kind event onto `video-Data` so `RoomManager.evict` can drop their sockets — a row marked without the socket closed leaves them receiving the whole broadcast. **A removed guest can rejoin by clearing their cookie**: guest identity is the `guestId` cookie and nothing survives it being re-rolled.
